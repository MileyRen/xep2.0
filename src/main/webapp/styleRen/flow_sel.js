...
function Flow(bindID, xmlStr, editable, title) {
	...
	(function () {		// init jsondata
		...
		for (var i = 0; i < nodes.length; i++) {
			...
			if(nodeInforDom.length) {
				...
				var color = nodeInforDom.getAttribute('color');
				//for(var l in 2) {
					//if(tools[l].id == toolId) {
					jsonNodes[id].color = color || 'white';
						//break;
					//}
				//}
			}
	...
	instance.onBtnSaveClick = function () {
		...
		for (var i in jsondata.nodes) {
			...
			if (!nodeInfor[i]) {
				alert('There are some nodes, which have no tool.');
				return false;
			}
		}
		...
		if(checkCircle()) {
			alert('There are several circle');
			return false;
		}
		...
		function checkCircle() {
			...
			var moved = [];
			var lineMoved = [];
			while(stack.length < instance.$nodeCount) {
				flag = false;
				for(var i in instance.$nodeData) {
					if(moved[i]) {
						continue;
					}
					var noIn = true;
			...
			if(stack.length != instance.$nodeCount) {
				return true;
			}
			return false;
		}
	};
	instance.onNodeDel = function (id) {
		delete nodeInfor[id];
	}
...
//			delete nodeInfor[id];
	...
	instance.onDblclickNode = function (idP, nodeName) {
		...
		if(idP == '_1') {
			...
			window.open('flow/detail2.action', 'Input Information', 'height=500, width=1030, top=0, left=0, toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, status=no');
			return;
		}
		if(idP == '_3') {
			instance.$editable = false;
			window.open('flow/detail3.action', 'Output Information', 'height=500, width=1030, top=0, left=0, toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, status=no');
			return;
		}
		...
		window.open('flow/modify2.action?id=' + encodeURIComponent(idPointer) + '&name=' + encodeURIComponent(nodeName), 'Node Information', 'height=500, width=800, top=0, left=0, toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, status=no');
	};
	...
	instance.onDblclickLine = function(idP) {
		...
		if(!nodeInfor || !nodeInfor[instance.$lineData[idP].from] || !nodeInfor[instance.$lineData[idP].to]) {
			alert('The nodes between the line have no tool.');
			return false;
		}
		...
		// 生成输入输出节点信息
		// 打开新的窗口
		var cWin = window.open('flow/modifyInputOutput.action?from=' + encodeURIComponent(instance.$lineData[idP].from) + '&to=' + encodeURIComponent(instance.$lineData[idP].to), 'Node Information', 'height=710, width=1000, top=0, left=0, toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, status=no');
		// 在窗口关闭后，如果要求保存
		// 获取修改的值，添加对应值的修改
		// 更新对应的表格
...
//			</bpmndi:BPMNShape>
	...
	function _setParamList(rootElement, params) {
		...
		$(table).append($('<tr><th>From</th><th>To</th></tr>'))
