...
//初始化区域图的对象
	...
	var headHeight = 0;
	var tmp = "";
	if (property.haveHead) {			// 我加入了titleDom，并在外面加入了监听器
		tmp = "<div class='GooFlow_head'><label id='titleDom' title='" + (property.initLabelText || "newFlow_1") + "'>" + (property.initLabelText || "newFlow_1") + "</label>";
		// 将工具放在了侧面
	...
	if (this.$editable) {
		this.$workArea.on("click", {inthis: this}, function (e) {
			...
			e.data.inthis.addNode("_jbpm-unique-" + e.data.inthis.$max, {
				name: e.data.inthis.$max,
				left: X,
				top: Y,
		...
		this.$lineOper = $("<div class='GooFlow_line_oper' style='display:none'><b class='b_x'></b></div>");//选定线时显示的操作框
		...
		// 我添加：在用户双击节点时发生，不会发生在开始和结束节点上。
		// function(idPointer, nodeName)
		this.onDblclickNode = null;
		// 我添加：在用户双击连接线时发生，功能是在开始和结束的时候。
		// function(idPointer)
		this.onDblclickLine = null;
		// 我添加：在用户画线完成后发生
		// function(idStart, idEnd)
		this.onLineEnd = null;
		// 我添加：在用户画线完成后发生
		// function(idP)
		this.onAfterLineEnd = null;
		this.onNodeDel = null;
...
GooFlow.prototype = {
	...
	initDraw: function (id, width, height) {
		...
		$(this.$draw).delegate(tmpClk, "dblclick", {inthis: this}, function (e) {
			...
			if(This.onDblclickLine) {
				This.onDblclickLine(This.$focus);
			}
	...
	addNode: function (id, json) {
		...
		if (json.type.indexOf(" round") < 0) {
			...
			this.$nodeDom[id]
				...
				"<tr>" +
				"<td class='ico'>" +
				"<b class='ico_" + json.type + "'"+ "style='background-color:"+ json.color +";'" +">" +
				"</b></td>" +
				"<td>" + json.name + "</td></tr>" +
				"</table>" +
	...
	initWorkForNode: function () {
		...
		this.$workArea.delegate(".GooFlow_item", "mouseup", {inthis: this}, function (e) {
			...
			if (lineStart && !This.$mpTo.data("p")) {
				// 我添加的
				if(This.onLineEnd && This.onLineEnd(lineStart.id, this.id)==false) {
					return ;
				}

				This.addLine("_jbpm-unique-" + This.$max, {
					from: lineStart.id,
					to: this.id,
					name: "",
					type: "tb",
				...
				if(This.onAfterLineEnd) {
					This.onAfterLineEnd("_jbpm-unique-" + This.$max);
				}
				This.$max++;
			}
			else {
...
//			pInstance.setName(pInstance.$textArea.data("id"), 'cc', "node")
			if(This.$editable && This.onDblclickNode) {
				This.onDblclickNode(This.$focus, oldTxt);
			}
//            var t = getElCoordinate(This.$workArea[0]);
//            This.$textArea.val(oldTxt).css({
...
//            });
	...
	//设置结点/连线/分组区域的文字信息
	// 我添加了addOn
	setName: function (id, name, type, addOn) {
		var oldName;
		if (type == "node") {//如果是结点
			if (!this.$nodeData[id])    return;
			...
			if (this.$nodeData[id].type.indexOf("round") > 1) {
				...
				if(addOn && addOn.color) {
					this.$nodeDom[id].find("td:eq(0)").children('b').css('backgroundColor', addOn.color);
				}
			...
			if (GooFlow.prototype.useSVG != "") {
				this.$lineDom[id].childNodes[2].innerHTML = "";
				this.$lineDom[id].childNodes[2].appendChild(name);
			}
			else {
	...
	delNode: function (id) {
		...
		if (this.$editable) {
			//在回退新增操作时,如果节点ID以"_jbpm-unique-"开头,则表示为本次编辑时新加入的节点,这些节点的删除不用加入到$deletedItem中
			if (id.indexOf("_jbpm-unique-") < 0)
	...
	addLineDom: function (id, lineData) {
		...
		if (GooFlow.prototype.useSVG == "") {
			this.$lineDom[id].childNodes[1].innerHTML = "";
			$(this.$lineDom[id].childNodes[1]).append(lineData.name);
			if (lineData.type != "sl") {
			...
			this.$lineDom[id].childNodes[2].innerHTML = "";
			$(this.$lineDom[id].childNodes[2]).append(lineData.name);
		}
	},
	/**
	 * 增加一条线
	...
	resetLines: function (id, node) {
		for (var i in this.$lineData) {
			...
			if (GooFlow.prototype.useSVG == "") {
				...
				this.$lineDom[i].childNodes[2].innerHTML = "";
				$(this.$lineDom[i].childNodes[2]).append(this.$lineData[i].name);
			}
		}
	},
	...
	setLineType: function (id, newType, M) {
		...
		//如果是变回直线
			...
			if (GooFlow.prototype.useSVG == "") {
				...
				this.$lineDom[id].childNodes[2].innerHTML = "";
				$(this.$lineDom[id].childNodes[2]).append(this.$lineData[id].name);
			}
		}
		if (this.$focus == id) {
			this.focusItem(id);
	...
	delLine: function (id) {
		...
		if (this.$editable) {
			//在回退新增操作时,如果节点ID以"_jbpm-unique-"开头,则表示为本次编辑时新加入的节点,这些节点的删除不用加入到$deletedItem中
			if (id.indexOf(this.$id + "_jbpm-unique-") < 0)
