...
public class FileMgrAction extends ActionSupport implements SessionAware, ModelDriven<FileAndFolder>, Preparable {
	private static final long serialVersionUID = -9085791873995676177L;
	@Autowired
	...
	public String moveFileOrFolder() throws Exception {
		...
		if (move) {
			...
			session.put("moveFileOrFolder", "Move Success!");
			return getPage();
		} else {
			logger.info("移动失败");
			session.put("moveFileOrFolder", "Move Failed!");
			return "error";
		}
	}
