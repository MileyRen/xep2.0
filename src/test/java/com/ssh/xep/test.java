...
public class test {
	...
	public void tes() throws FileNotFoundException {
		/*
		 * User user = new User(); user.setId(1); String rootPath =
		 * PathFormat.strEnd("D:\\xeptest"); String mappingpath =
		 * PathFormat.strEnd("D:\\xeptest") +
		 * folderService.getAll("From FileAndFolder where userId=" + 1 +
		 * " and type='mapping'").get(0).getName(); List<File> files =
		 * folderService.scanMappingPath(mappingpath); for (File file : files) {
	...
	/*
	 * @Test public void teses(){ FileAndFolder f2File = new FileAndFolder();
	 * f2File.setName("folder22"); f2File.setParentFolderId(-1);
	 * f2File.setSize(""); f2File.setTime(new Date()); f2File.setType("folder");
	 * f2File.setUserId(1); FileAndFolder deleteFlag =
	 * folderService.getById(-1); f2File.setDeleteFlag(deleteFlag);
